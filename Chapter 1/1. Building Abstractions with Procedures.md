...
Lisp 並不是一個精心設計的成果，相反地，它是循著用戶需求的反饋和實用上的考量，以一種實驗性質的方式逐漸地演化而成的。Lisp 的非正式演化已經持續了多年，而且 Lisp 社群傳統上刻意的抗拒去頒布一個官方正式定義的 Lisp 語言。 伴隨著 Lisp 最初構想的彈性和優雅，演化使得 Lisp 這個世界上第二老的語言 (目前尚且廣為使用的語言只有 Fortran 比它老)  得以不斷的適應包含最新的程式設計概念。因此，當今的 Lisp 其實是一個有許多方言的語言家族，這個家族的成員共有 Lisp 的最初構想，但是可能在許多地方有顯著的不同。 本書所採用的 Lisp 方言叫做 Scheme 。
 ...
 Lisp 一開始因為其實驗性格和著重符號處理，它數字計算非常慢，至少跟 Fortran 比是如此。但是經過了許多年 Lisp 編譯器的發展，現在將程式轉換為機器碼後已經具備了合理的數字計算效率。
 在一些特殊的應用中，Lisp 有巨大的功效。
